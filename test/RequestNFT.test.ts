...
describe("SendCrypto Contract", () => {
  ...
  beforeEach(async () => {
    ...
    testERC721TokenContract = (await TestERC721Token.deploy()) as TestERC721Token;
  ...
  it("Can exchange", async () => {
    await testERC20TokenContract.connect(deployer).approve(requestNFTContract.address, ethers.utils.parseEther("100"));
    expect(await testERC20TokenContract.connect(deployer).allowance(deployer.address, requestNFTContract.address)).to.be.equal(ethers.utils.parseEther("100"));
    expect(await testERC721TokenContract.connect(bob).balanceOf(bob.address)).to.be.equal(1);
    expect(await testERC721TokenContract.connect(bob).ownerOf(1)).to.be.equal(bob.address);
    const types = {
        Buy: [
      ...
      let digest = await deployer._signTypedData(
        domainData,
        types,
        data
      )
      ...
      await testERC721TokenContract.connect(bob).approve(requestNFTContract.address, 1);
      await requestNFTContract.connect(bob).exchange(data, v, r, s);

        
    expect(await testERC20TokenContract.connect(bob).balanceOf(bob.address)).to.be.equal(ethers.utils.parseEther("50"));
    expect(await testERC20TokenContract.connect(deployer).balanceOf(deployer.address)).to.be.equal(ethers.utils.parseEther("950"));
    expect(await testERC721TokenContract.connect(deployer).ownerOf(1)).to.be.equal(deployer.address);
  });
  ...
  it("Cannot exchange after time expiry", async () => {
    ...
    const types = {
      ...
      await testERC721TokenContract.connect(bob).approve(requestNFTContract.address, 1);
      expect(requestNFTContract.connect(bob).exchange(data, v, r, s)).to.be.revertedWith("RequestNFT:exchange: offer has expired");
  });
